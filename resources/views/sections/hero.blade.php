...
<section
    class="relative flex min-h-[100svh] items-center overflow-hidden bg-brand-darker pt-16 lg:pt-20"
    id="home"
>
    ...
    <div class="absolute inset-0 bg-gradient-to-b from-brand-darker/65 via-black/45 to-brand-darker/80"></div>
    <div class="absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-brand-darker/95 to-transparent pointer-events-none"></div>

    <div class="relative z-20 mx-auto w-full max-w-6xl px-5 py-20 text-center sm:px-6 sm:py-24 lg:px-8 lg:py-28">
        <p class="mb-5 text-xs font-semibold uppercase tracking-[0.22em] text-brand-accent/90 sm:text-sm sm:tracking-[0.28em]">
            Prestige In Media
        </p>

        <h1 class="mx-auto max-w-4xl text-4xl font-bold leading-[1.08] tracking-tight text-white text-balance sm:text-5xl lg:text-6xl">
            {!! $heroTitle !!}
        </h1>

        @if ($hero?->subtitle)
            <p class="mx-auto mt-6 max-w-3xl text-base leading-relaxed text-white/76 text-pretty sm:mt-8 sm:text-lg">
                {{ $hero->subtitle }}
            </p>
        @endif
